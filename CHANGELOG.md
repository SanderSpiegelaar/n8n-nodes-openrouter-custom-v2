...
- Added Web Search Plugin configuration (enable toggle, optional `max_results` 1–10, optional `search_prompt`) emitting `plugins[{ id: 'web', ... }]` on the wire. Coexists with the response-healing plugin in a single `plugins` array. Pre-request validator rejects `:online` model variant combined with the web plugin.
